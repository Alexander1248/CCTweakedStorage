...
local is_LuaJIT = false  -- LuaJIT 1.x.x and Luau are treated as vanilla Lua 5.1/5.2
...
if branch == "FFI" or branch == "LJ" or branch == "LIB32" then
   ...
   SHL  = b.lshift or b.blshift              -- second argument is integer 0..31
   SHR  = b.rshift or b.blogic_rshift              -- second argument is integer 0..31
   ROL  = b.rol or b.lrotate or function(v, n) return OR(SHL(v, n), SHR(v, 32 - n)) end    -- second argument is integer 0..31
   ROR  = b.ror or b.rrotate or function(v, n) return OR(SHR(v, n), SHL(v, 32 - n)) end    -- second argument is integer 0..31
   NOT  = b.bnot                -- only for LuaJIT
   NORM = b.tobit               -- only for LuaJIT
   HEX  = b.tohex or function(v) 
      val = "0123456789abcdef"
      str = "" 
      div = v
      while div > 0 do
         rem = b.band(div, 15) + 1
         str = str .. val:sub(rem, rem)
         div = SHR(div, 4)
      end
      return str
